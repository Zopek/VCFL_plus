...
from aligned_reid.model.Model_fmr import Model
...
class Config(object):
  def __init__(self):
    ...
    parser.add_argument('-alw', '--align_loss_weight', type=float, default=0.)    
    ...
    self.local_conv_out_channels = 1024
    ...
    if args.exp_dir == '':
      self.exp_dir = osp.join(
        ...
        'alw_{}_'.format(tfs(self.align_loss_weight)) +                               
...
class ExtractFeature(object):
  ...
  def __call__(self, ims):
    ...
    feat, feat_part1, feat_part2, global_feat, local_feat = self.model(ims)[:5]
    feat = feat.data.cpu().numpy()
    feat_part1 = feat_part1.data.cpu().numpy()
    feat_part2 = feat_part2.data.cpu().numpy()                
    global_feat = global_feat.data.cpu().numpy()
    local_feat = local_feat.data.cpu().numpy()
    # Restore the model to its old train/eval mode.
    self.model.train(old_train_eval_model)
    return feat, global_feat, local_feat #feat_part1, feat_part2, feat_part3, feat_part4,
